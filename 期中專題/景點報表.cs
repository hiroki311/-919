...
namespace 期中專題
{
    public partial class 景點報表 : Form
    {
        ...
        {
            string[] photo = new string[] { "未選擇", "亞洲", "大洋洲", "歐洲", "非洲", "美洲" };
        ...
        private void 新增_Click(object sender, EventArgs e)
        {
            ...
            var q = from p in tr.各地區名稱總表
                    where p.各地地區名稱 == 地區.Text
                    select p.各地地區名稱ID;

            國家名稱 list = new 國家名稱();
            {
                list.國家名稱1 = txt_國家名稱.Text;
                list.各地地區名稱ID = q.FirstOrDefault();
            }
            //if (list.國家名稱1 == txt_國家名稱.Text)
            //{
            //    MessageBox.Show(list.國家名稱1.ToString());
                
            //    MessageBox.Show("已有此國家資料，不須新增");
            //    return;
            //}
        ...
        private void 地區_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (地區.SelectedItem.ToString() == "歐洲")
            {
                var q = from p in tr.國家名稱
                        where p.各地地區名稱ID == 3
                        select p.國家名稱1;
            ...
            else if (地區.SelectedItem.ToString() == "非洲")
            {
                var q = from p in tr.國家名稱
                        where p.各地地區名稱ID == 4
                        select p.國家名稱1;
            ...
            else if (地區.SelectedItem.ToString() == "美洲")
            {
                var q = from p in tr.國家名稱
                        where p.各地地區名稱ID == 5
                        select p.國家名稱1;
        ...
        {
            txtcityname.Text = dataGridView1.CurrentCell.OwningRow.Cells["城市名稱"].Value.ToString();
        }
